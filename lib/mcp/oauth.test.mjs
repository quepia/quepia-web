...
test("parsea una decisión de formulario mínima", () => {
  ...
  assert.equal(
    parseOAuthDecisionRequest(
      `authorization_id=${AUTHORIZATION_ID}&decision=approve&redirect_uri=https%3A%2F%2Fevil.example`,
    ),
    null,
  )
  assert.equal(
    parseOAuthDecisionRequest(
      `authorization_id=${AUTHORIZATION_ID}&authorization_id=other&decision=deny`,
    ),
    null,
  )
  assert.equal(
    parseOAuthDecisionRequest(
      `authorization_id=${AUTHORIZATION_ID}&decision=grant`,
    ),
    null,
  )
})
