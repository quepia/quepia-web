...
test("devuelve al cliente OAuth sin depender de un redirect sujeto a form-action", () => {
  const document = buildOAuthRedirectDocument(
    "https://claude.ai/api/mcp/auth_callback?code=abc&state=xyz",
  )

  assert.match(
    document,
    /<meta http-equiv="refresh" content="0; url=https:\/\/claude\.ai\/api\/mcp\/auth_callback\?code=abc&amp;state=xyz">/,
  )
  assert.match(
    document,
    /<a href="https:\/\/claude\.ai\/api\/mcp\/auth_callback\?code=abc&amp;state=xyz"/,
  )
  assert.doesNotMatch(document, /<script/i)
})

test("escapa el destino antes de incrustarlo en el documento de retorno", () => {
  const document = buildOAuthRedirectDocument(
    'https://ai.example/callback?code=a"><script>alert(1)</script>',
  )
...
test("rechaza destinos que no son redirects OAuth permitidos", () => {
  assert.throws(
    () => buildOAuthRedirectDocument("javascript:alert(document.domain)"),
    /INVALID_OAUTH_REDIRECT/,
  )
  assert.throws(
    () => buildOAuthRedirectDocument("http://ai.example/callback"),
    /INVALID_OAUTH_REDIRECT/,
  )
})
