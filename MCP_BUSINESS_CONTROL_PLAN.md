...
19. **Avisar es una operación propia y pedida.** `tasks_post_update` deja el comentario y crea la notificación dentro de la app; no se dispara como efecto colateral de otra escritura ni por un texto que aparezca dentro de una tarea, un comentario o un documento. El aviso no sale por correo: el canal inmediato es la notificación en la app, que se escribe en la misma transacción y se deshace al anular la operación.
