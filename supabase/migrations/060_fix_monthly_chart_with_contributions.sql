-- =====================================================
-- FIX: incluir aportes/devoluciones en gráfico mensual
-- Migración: 060_fix_monthly_chart_with_contributions.sql
-- Objetivo: mantener consistente el cálculo de "Sin distribuir"
-- =====================================================
...
BEGIN
  ...
  income_data AS (
    ...
    FROM (
      ...
      -- Aportes de socios (también ingresan dinero a caja)
      SELECT
  ...
  expense_data AS (
    ...
    FROM (
      ...
      -- Devoluciones de aportes (salida de dinero)
      SELECT
