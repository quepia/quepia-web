-- Migration 030: Add Social Copy Field & Update RPCs
-- Adds: social_copy column to tasks, updates client data fetching RPCs

-- 1. Add social_copy column to sistema_tasks
ALTER TABLE sistema_tasks ADD COLUMN IF NOT EXISTS social_copy TEXT;

-- 2. Update get_client_project_data (V1/Legacy) to include social_copy
...
BEGIN
    ...
    -- 4. Get tasks if allowed (NOW WITH ASSETS and SOCIAL_COPY)
...
-- 3. Update get_client_project_data_v2 (V2/Session) to include social_copy
-- Assuming V2 structure is similar, but uses session_uuid.
-- I'll copy the logic from V1 but adapt for V2 based on context or previous knowledge.
-- Re-checking V2 RPC name in user context: get_client_project_data_v2
...
BEGIN
    ...
    -- Update last accessed for client
    UPDATE sistema_client_access
    SET last_accessed = NOW()
    ...
    -- 4. Get Tasks (NOW WITH ASSETS and SOCIAL_COPY)
